...
describe(`AuthHttpInterceptor`, () => {
    ...
    it('should obtenerErrorHttpCode return "Error inesperado en la petición" if error has unknown property httpCode', () => {
        const response = (manejador as any).obtenerErrorHttpCode(406);

        expect(response).toEqual('Error inesperado en la petición');
    });

    it('should obtenerErrorHttpCode return respective error message if error has known property httpCode', () => {
        const response = (manejador as any).obtenerErrorHttpCode(404);
